    ...
    def figure_atmos(self):
        ...
        if self.kstest == True:
            ...
            for j in range(0, len(variable)):
                ...
                print(f" - {rounder(self.gks_result[j])}% of the raw GCM are likely to be drawn from the same distribution.")
                print("")
                print(f"• KS test for bias-corrected GCM")
                print(f" - {rounder(self.dks_result[j])}% of the bias-corrected GCM are likely to be drawn from the same distribution.")
                print("")
                print("• Result")
                print(f" - The bias-corrected GCM presents {rounder(self.dks_result[j])-rounder(self.gks_result[j])}% improvement compared to the raw GCM.")
                print("")
